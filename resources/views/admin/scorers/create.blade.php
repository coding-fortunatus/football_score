								...
								<div class="col-sm-12 col-12">
										...
										<div class="card">
												<div class="card-header">
														<div class="card-title">Register Goal Scorer</div>
												</div>
												<div class="card-body">
														<!-- Form start -->
														<form action="{{ route('scorers.store') }}" method="POST">
																@csrf
																<div class="row">
																		<div class="col-12">
																						...
																						<select class="form-select" id="match_id" name="match_id">
																								...
																								@foreach ($matches as $match)
																										<option value="{{ $match->id }}" {{ old('match_id') == $match->id ? 'selected' : '' }}>
																												{{ $match->schedule->league->title }}: {{ $match->schedule->team1->short_name }} vs
																		...
																		<div class="col-xl-4 col-sm-6 col-12">
																				<div class="mb-3">
																						...
																						<input type="number" class="form-control" id="minute" name="minute"
																								value="{{ old('minute') }}" placeholder="Enter the minute goal was scored">
																						@error('minute')
																								<small class="text-danger">{{ $message }}</small>
																						@enderror
																				</div>
